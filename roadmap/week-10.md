...
1. 运行 `pnpm harness:demo`。
2. 增加一个只读工具和一个需要审批的工具。
3. 制造预算耗尽，观察 trace。
4. 写一份 observability 选型记录：Langfuse、LangSmith、OpenTelemetry 的取舍。
5. 给 AI 学习助手定义 trace 字段：run id、model、retrieval、tool call、latency、token/cost、eval score。
