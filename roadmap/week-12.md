...
| Docusaurus 发布 | 学习资料是否可阅读和发布 | `site/sidebars.ts` |
...
- 工具权限和人工审批。
- eval 是否进入 CI。
- GitHub Pages/Docusaurus 文档是否同步。

本路线不要求 Week 12 完成生产部署，但要知道缺口在哪里。

### Docusaurus 发布

本仓库已经用 Docusaurus 友好的结构组织：

- `README.md` 是首页。
- `site/sidebars.ts` 是左侧目录。
- `site/docusaurus.config.ts` 指定站点 URL、baseUrl、导航和 docs 入口。
- 每周学习内容在 `roadmap/week-*.md`。
- labs、harness、eval、agent workflows 都有文档入口。

后续推到 GitHub 后，GitHub Actions 会构建 Docusaurus 并发布到 GitHub Pages。
...
检查 `site/sidebars.ts` 是否覆盖所有阅读入口。
...
- README、首页和 sidebars 能指导新学习者按教程顺序阅读。
- 能说清楚当前项目距离生产可用还差什么。
