...
1. 运行 `pnpm eval`。
2. 新增一个失败 case，并记录为什么失败。
3. 用 checkpoint 保存一次 run，再读取它。
4. 写一个 PromptFoo 配置草案，说明如何把现有 JSON eval 迁移过去。
5. 为 RAG 场景标注 faithfulness、answer relevancy、context recall 分别需要哪些输入。
