...
| LangGraph replay | 图状态如何恢复、重放和分叉 | checkpoint/time travel 对照 |
...
LangGraph 的 time travel 思路适合帮助理解 replay：从某个 checkpoint 恢复，节点之前的结果不重新执行，节点之后的步骤可以重新跑，也可以修改状态后 fork 出一个新路径。

这对 agent 调试很重要：

- 如果检索结果错了，应该从 retrieve 前重放。
- 如果审批后动作失败，应该从 approval 后重放。
- 如果模型回答不稳定，应该固定检索和工具结果，只重放生成节点。
- 如果外部工具有副作用，replay 必须使用 mock 或 recorded result。

把这个思想迁移回本仓库，就是要求 checkpoint 和 trace 记录足够多的信息，让失败不只是“再跑一次试试”。
...
用 LangGraph checkpoint/time travel 的思路，设计一次失败 replay：从哪个状态恢复、哪些节点重跑、哪些工具结果 mock、如何比较 trace。
...
- 能说明 LangGraph checkpoint/time travel 对 agent failure recovery 的启发。
