...
| LangChain.js 对照 | 成熟框架如何组织 RAG pipeline | loader/splitter/retriever 映射 |
...
LangChain.js 值得在本周作为对照学习，但不建议一开始就替换本仓库 baseline。先把自建 RAG 的每个步骤映射到框架概念：

| 本仓库 baseline | LangChain.js 概念 | 需要观察的问题 |
| --- | --- | --- |
| Markdown 文件扫描 | document loader | 元数据、路径、标题是否保留 |
| 标题和正文合并切分 | text splitter | chunk 是否保留语义边界 |
| `searchLearningNotes` | retriever | query 改写、topK、score 如何控制 |
| 本地关键词检索 | BM25/keyword retriever | 专有名词和周次是否稳定命中 |
| 未来向量检索 | vector store retriever | embedding 召回是否优于关键词 |
| answer sources contract | retrieved context + citation policy | 回答是否只依据来源 |

学习目标不是记住 LangChain API，而是看懂一个 RAG 框架必须解决哪些工程问题：文档加载、切分、索引、召回、重排、上下文组装、引用和 eval。
...
写一份 LangChain.js RAG 对照笔记：loader、splitter、retriever、vector store 分别对应本仓库哪段逻辑。
...
- 能说明什么时候不应该回答。
- 能说明 LangChain.js RAG pipeline 和本仓库 baseline 的概念映射。
