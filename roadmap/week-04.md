...
1. 运行 `pnpm lab:rag`。
2. 用 MCP 角色相关问题、`第 10 周 Harness`、未知主题三类 query 测试检索。
3. 写一份向量数据库选型记录：本地 Chroma，生产 pgvector，托管 Pinecone。
4. 设计 hybrid retrieval：关键词召回、向量召回、rerank、context packing。
5. 新增一个 eval case，要求问题必须命中正确 roadmap 来源。
